...
test.describe('Publish Paid API Product Workflow with Endpoint Pricing - Tier', () => {

  test('should successfully automate Create and Publish Paid API Product with Endpoint Pricing - Tier workflow', async () => {
    let categoryId;
    let prefix;
    let projectId;
    let groupId;
    ...
    // Configure Endpoint Pricing (Tier)
    await test.step('Configuring Endpoint Pricing (Tier)...', async () => {
      expect(projectId).toBeDefined();
      expect(endpointId).toBeDefined();
      console.log(`Configuring Endpoint Pricing (Tier)...`);
      ...
      if (body) {
        if (body.data) {
          monetizationId = body.data.id || (body.data.tiers && body.data.tiers[0] && body.data.tiers[0].id);
          pricingPlanId = body.data.pricing_plan_id || body.data.id;
        }
      }
      console.log(`Endpoint Pricing (Tier) configured successfully. Monetization ID: ${monetizationId}, Plan ID: ${pricingPlanId}`);
    });
    ...
    await test.step('POST /portaldev/api/vendor/projects/{projectId}/versions/submit -> Submit Version', async () => {
      ...
      console.log(`[DEBUG ID Log] monetization IDs: ${monetizationId}`);
