...
const createRequestBasedPlan = require('../../pricing-plan/plan-base-on-No.-of-request');
...
const projectName = `Paid_BYOK_Product_${pricingPlan.replace(/[\s-]+/g, '_')}`;
...
test.describe('Publish Paid BYOK API Product Workflow with Request-Based Plan', () => {

  test('should successfully automate Create and Publish Paid BYOK API Product with Request-Based Plan workflow', async () => {
    let categoryId;
    let prefix;
    let projectId;
    let groupId;
    ...
    // Create Request-Based Pricing Plan
    await test.step('Create Request-Based Pricing Plan', async () => {
      expect(projectId).toBeDefined();
      console.log(`Creating Request-Based Pricing Plan...`);
      const planResponse = await createRequestBasedPlan(projectId);
      expect(planResponse).toBeDefined();
      
      const body = planResponse.body;
      if (body) {
        if (body.data && body.data.id) {
          pricingPlanId = body.data.id;
        } else if (Array.isArray(body.data)) {
          const p = body.data.find(p => p.type === 'plan');
          if (p) pricingPlanId = p.id;
        } else if (Array.isArray(body)) {
          const p = body.find(p => p.type === 'plan');
          if (p) pricingPlanId = p.id;
        }
      }
      console.log(`Request-Based Pricing Plan created/retrieved successfully. ID: ${pricingPlanId}`);
    });
    ...
    // Step 10.5: Send PATCH /api/projects/{projectId} -> Update with is_byok_enabled = true
    await test.step('PATCH /api/projects/{projectId} -> Enable BYOK', async () => {
      expect(projectId).toBeDefined();
    ...
    await test.step('POST /api/vendor/projects/{projectId}/versions/submit -> Submit Version', async () => {
      ...
      // Log debugging requirements before Submit Version
      console.log(`[DEBUG ID Log] projectId: ${projectId}`);
      console.log(`[DEBUG ID Log] versionId: ${versionId}`);
      console.log(`[DEBUG ID Log] groupId: ${groupId}`);
      console.log(`[DEBUG ID Log] endpointId: ${endpointId}`);
      console.log(`[DEBUG ID Log] pricingPlanId: ${pricingPlanId}`);
      console.log(`[DEBUG ID Log] monetization IDs: ${pricingPlanId}`);
