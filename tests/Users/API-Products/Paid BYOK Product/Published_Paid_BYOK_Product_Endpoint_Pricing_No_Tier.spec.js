...
test.describe('Publish Paid BYOK API Product Workflow with Endpoint Pricing - No Tier', () => {

  test('should successfully automate Create and Publish Paid BYOK API Product with Endpoint Pricing - No Tier workflow', async () => {
    let categoryId;
    let prefix;
    let projectId;
    let groupId;
    ...
    // Step 11.5: Configure Endpoint Pricing (No Tier)
    await test.step('Configuring Endpoint Pricing (No Tier)...', async () => {
      expect(projectId).toBeDefined();
      expect(endpointId).toBeDefined();
      console.log(`[Step 11.5] Configuring Endpoint Pricing (No Tier)...`);
      const planResponse = await configureEndpointPricingWithoutTier(projectId, endpointId);
      expect(planResponse).toBeDefined();
      console.log(`[Step 11.5] Endpoint Pricing (No Tier) configured successfully.`);
    });
