...
const createPackagePlan = require('../../pricing-plan/plan-as-a-package');
...
const pricingPlan = 'Package Plan';
...
test.describe('Publish Paid BYOK API Product Workflow with Package Plan', () => {

  test('should successfully automate Create and Publish Paid BYOK API Product with Package Plan workflow', async () => {
    let categoryId;
    let prefix;
    let projectId;
    let groupId;
    ...
    await test.step('Create Package Pricing Plan', async () => {
      expect(projectId).toBeDefined();
      console.log(`Creating Package Pricing Plan...`);
      const planResponse = await createPackagePlan(projectId);
      expect(planResponse).toBeDefined();
      console.log(`Package Pricing Plan created successfully.`);
    });
