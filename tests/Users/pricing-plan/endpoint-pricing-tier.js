...
/**
 * Configures endpoint pricing with tiers.
 *
 ...
 */
async function configureEndpointPricingWithTier(projectId, endpointId) {
  const helperName = 'endpoint-pricing-tier';
  if (!projectId) {
  ...
  console.log(`[${helperName}] [Step 3] Configuring tier-based endpoint pricing at POST ${configurePricingUrl} with payload:`, JSON.stringify(pricingPayload, null, 2));
  ...
  if (pricingResponse.status < 200 || pricingResponse.status >= 300) {
    console.error(`[${helperName}] [Step 3] Configure tier pricing failed. Response body:`, JSON.stringify(pricingResponse.body, null, 2));
    throw new Error(`[${helperName}] [Step 3] Configure tier pricing failed with status ${pricingResponse.status}`);
  }
  ...
  console.log(`[${helperName}] [Step 3] Endpoint pricing (with tiers) configured successfully.`);
  return pricingResponse;
}

module.exports = configureEndpointPricingWithTier;
